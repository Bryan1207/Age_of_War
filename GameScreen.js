function GameScreen(diffeculty) {
	if(DEV_MODE) console.log("Welkom bij het GameScreen");
	...
	this.timerAI = 0;
	this.timerAIMax = 3 * 60;
	this.timerAILevel = 0;
	this.timerAILevelCount = 0;
	this.winner = false;
	this.loser = false;
}
		...
		if(this.menuChoise === 0) {
			...
			for(var i=0; i<this.turretCount; i++) {
				if(ClickOnItem(this["turret" + i + "_evolve"+this.level].x, this["turret" + i + "_evolve"+this.level].y, 
					...
					if(this.player.CanIAffortIt(TURRET_DATA[this.level-1][i].COSTS)) {
...
GameScreen.prototype.Draw = function() {
	...
	if(this.winner) {
		...
		ctx.fillText("You Won!", 600, 200);
	} else if(this.loser) {
		ctx.fillStyle = "#FFFFFF";
		ctx.textAlign = 'center';
		ctx.font = "bold 100px cursive";
		ctx.fillText("You Lost!", 600, 200);
	}
}
	...
	if(this.enemiesOnField.length > 0) {
		for(var i=0; i<this.enemiesOnField.length; i++) {
			if(this.unitsOnField.length > 0) {
				...
				/*if(this.enemiesOnField[i].GetCanShoot()) {
					if(this.enemiesOnField[i].GetPosition().x <= (this.unitsOnField[0].GetPosition().x + this.unitsOnField[0].GetWidth()) + 100) {
						this.enemiesOnField[i].PlayAnimation("shoot");
						if(this.enemiesOnField[i].GetAnimation().getFinished()) {
							this.enemiesOnField[i].GetAnimation().ResetAnimation();
							this.unitsOnField[0].RemoveHP(this.enemiesOnField[i].GetDamage()/10);
...
GameScreen.prototype.UpdateUnits = function() {
	if(this.unitsOnField.length > 0) {
		for(var i=0; i<this.unitsOnField.length; i++) {
			if(this.enemiesOnField.length > 0) {

				/*if(this.unitsOnField[i].GetCanShoot()) {
					if((this.unitsOnField[i].GetPosition().x + this.unitsOnField[i].GetWidth()) + 100 >= this.enemiesOnField[0].GetPosition().x) {
						this.unitsOnField[i].PlayAnimation("shoot");
						if(this.unitsOnField[i].GetAnimation().getFinished()) {
							this.unitsOnField[i].GetAnimation().ResetAnimation();
							this.enemiesOnField[0].RemoveHP(this.unitsOnField[i].GetDamage()/10);
							if(this.enemiesOnField[0].GetHP() <= 0) {
								this.player.AddGold(UNITS[this.enemiesOnField[0].GetLevel()][this.enemiesOnField[0].GetUnitNr()].gold_back);
				...
				if(this.unitsOnField[0].GetPosition().x + this.unitsOnField[0].GetWidth() >= this.enemiesOnField[0].GetPosition().x) {
					...
					if(this.unitsOnField[0].GetAnimation().getFinished()) {
						...
						if(this.enemiesOnField[0].GetHP() <= 0) {
							...
							this.player.AddGold(UNITS[this.enemiesOnField[0].GetLevel()][this.enemiesOnField[0].GetUnitNr()].gold_back);
							this.player.AddExperience(UNITS[this.enemiesOnField[0].GetLevel()][this.enemiesOnField[0].GetUnitNr()].xp);
							this.RemoveUnitOnField(this.enemiesOnField[0]);
							this.enemiesOnField.shift(0);
							if(this.enemiesOnField.length > 0) {
								this.enemiesOnField[0].SetSpeed(this.enemiesOnField[0].GetDefaultSpeed());
...
GameScreen.prototype.UpdateSpecialAttack = function() {
...
GameScreen.prototype.DrawProgressbarUnit = function() {
	ctx.fillStyle = "#FF0000";
	ctx.fillRect( 237, 13,
...
/* Buttons */
GameScreen.prototype.DrawButtons = function() {
	for(var i=0; i<5; i++) {
...
GameScreen.prototype.DrawSpecialAttack = function() {
...
GameScreen.prototype.DrawUnitButtons = function() {
	for(var i=0; i<this.unitCount; i++) {
...
GameScreen.prototype.DrawTurretButtons = function() {
	for(var i=0; i<this.turretCount; i++) {
...
GameScreen.prototype.addLoadingUnit = function(unitnr) {
	if(this.loadingUnits.length <= 0) {
		this.loadingUnitsTimer = 0;
	}
	this.loadingUnits.push(unitnr);
};
GameScreen.prototype.loadAddedUnits = function() {
	if(this.loadingUnits.length > 0) {
...
GameScreen.prototype.addUnitOnField = function(unitnr) {
...
GameScreen.prototype.addEnemiesOnField = function(unitnr) {
...
GameScreen.prototype.RemoveUnitOnField = function(unit) {
...
var timeOut = null;
GameScreen.prototype.SetInfoText = function(text, timer) {
...
GameScreen.prototype.GetPlayer = function() {
	return this.player;
};
GameScreen.prototype.GetUnitsOnField = function() {
	return this.unitsOnField;
};
GameScreen.prototype.GetEnemiesOnField = function() {
	return this.enemiesOnField;
};
GameScreen.prototype.RemoveEnemyFromList = function(i) {
	this.enemiesOnField.shift(i);
};

GameScreen.prototype.EnemyAI = function() {
	this.timerAI++;
	if(this.timerAI >= this.timerAIMax) {
		this.timerAI = 0;
		this.timerAIMax = GetRandomInt(1, (3*this.diffeculty)) * 60;
		this.timerAILevelCount++;
		if(this.timerAILevelCount > 20) {
			if(this.timerAILevel < UNITS[levelManager.GetLevel()].length-1) {
				this.timerAILevel++;
				this.timerAILevelCount = 0;
				//console.log(this.timerAILevel);
			} else {
				this.timerAILevel = 0;
				if(this.enemy.GetLevel() < 1) {
					this.enemy.Evolve(this.enemy.GetLevel()+1);
				}
				//Reset turret?
			}
		}
		this.addEnemiesOnField(GetRandomInt(0, this.timerAILevel));
	}
